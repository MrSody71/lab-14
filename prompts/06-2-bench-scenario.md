Напиши бенчмарк-сценарий, который:
1. Запускает мок-сервер OWM
2. Прогоняет Go-коллектор и Python asyncio-коллектор
   с одинаковой нагрузкой
3. Замеряет throughput, latency, CPU, память
4. Сохраняет результаты в JSON

=== bench/scenarios/run_bench.py ===
...
prompts/06-2-bench-scenario.md — этот промт целиком.
