Создай главный скрипт pipeline, который соединяет
все компоненты вместе.

=== py-analyzer/src/analyzer/pipeline.py ===
...
ПРОВЕРКА:
  cd py-analyzer
  # Запустить в mock-режиме на 5 секунд
  timeout 5 uv run python -m analyzer.pipeline --mock 2>&1 || true

  # На Windows:
  # $p = Start-Process uv -ArgumentList "run","python","-m",
  #      "analyzer.pipeline","--mock" -PassThru
  # Start-Sleep 5
  # Stop-Process -Id $p.Id -Force

  # Проверить что файлы создались:
  ls data/ 2>/dev/null || dir data 2>$null

  # Запустить все тесты
  uv run pytest tests/ -v \
    --ignore=tests/test_arrow_integration.py \
    -m "not integration" 2>&1
  cd ..

Все тесты зелёные. Pipeline должен запуститься,
напечатать "Pipeline running" и корректно завершиться.

prompts/05-5-pipeline.md — этот промт целиком.
